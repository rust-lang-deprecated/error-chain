...
//!   optional, boxed `core::Error + Send + 'static` object
...
//!   `chain_err` method. This method on all `core::Error + Send + 'static`
...
//!   to match.
//! * Because the error type contains `core::Error + Send + 'static` objects,
...
//! ::core::result::Result<T, Error>`. Note that in both cases
...
//! error type implements `core::Error + Send + 'static`, as long as
...
//! above all errors deriving from the `core::fmt::Error` type will be
//! presented to the user as a new `ErrorKind::Fmt` variant, and the
//! cause will be the original `core::fmt::Error` error. In contrast, when
...
pub struct ErrorChainIter<'a>(pub Option<&'a types::error::Error>);

impl<'a> Iterator for ErrorChainIter<'a> {
    type Item = &'a types::error::Error;

    fn next<'b>(&'b mut self) -> Option<&'a types::error::Error> {
        match self.0.take() {
            Some(e) => {
                self.0 = e.cause();
                Some(e)
...
/// macro.
pub trait ChainedError: types::error::Error + Send + 'static {
    ...
    fn with_chain<E, K>(error: E, kind: K) -> Self
        where Self: Sized,
              E: types::error::Error + Send + 'static,
    ...
    #[cfg(feature = "backtrace")]
    #[doc(hidden)]
    fn extract_backtrace(e: &(types::error::Error + Send + 'static)) -> Option<Arc<Backtrace>>
        where Self: Sized;
}
...
pub struct State {
    ...
    pub next_error: Option<Box<types::error::Error + Send>>,
...
impl State {
    ...
    #[cfg(feature = "backtrace")]
    pub fn new<CE: ChainedError>(e: Box<types::error::Error + Send>) -> State {
