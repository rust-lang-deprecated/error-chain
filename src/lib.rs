...
/// macro.
pub trait ChainedError<S: State>: error::Error + Send + Sync + 'static {
    ...
    where
          Self: Sized,
          E: ToError + ::std::error::Error + Send + Sync + 'static,
    ...
    #[doc(hidden)]
    fn new(kind: Self::ErrorKind, state: InternalState<S>) -> Self
    where
        Self: Sized;
...
pub trait ToError {
    ...
    fn to_error_send(&self) -> &(error::Error + Send)
        where Self: error::Error + Send
    {
        self
    }
}
...
#[derive(Debug)]
pub struct DisplayChain<'a, T: 'a + ?Sized, S: State>(&'a T, PhantomData<S>);
...
#[derive(Debug)]
#[doc(hidden)]
pub struct InternalState<S: State> {
    /// The user's state type generated by the `error_chain` macro
    pub inner: S,
    /// Backtrace for the current error.
    pub backtrace: InternalBacktrace,
}

impl<S: State> Default for InternalState<S> {
    #[cfg(feature = "backtrace")]
    fn default() -> Self {
        InternalState {
...
impl<S: State> InternalState<S>
    where S::Error: error::Error + Send + Sync,
{
    /// Creates a new State type
    pub fn new<CE: ChainedError<S>>(e: Box<S::Error>) -> InternalState<S> {
    ...
    pub fn next_error(&self) -> &Option<Box<S::Error>> {
        self.inner.next_error()
    }
}
