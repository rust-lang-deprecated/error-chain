...
//! [`display_chain`]: example_generated/struct.Error.html#method.display_chain
...
impl<'a> fmt::Display for DisplayChain<'a>
{
    fn fmt(&self, fmt: &mut fmt::Formatter) -> fmt::Result {
        ...
        try!(writeln!(fmt, "Error: {}", self.error));

        for e in Iter::new(Some(self.error)).skip(1) {
...
impl State {
    ...
    pub fn new(e: Box<error::Error + Send>, bt: Option<InternalBacktrace>) -> State {
        let backtrace = bt.unwrap_or_else(InternalBacktrace::new);
        State {
            next_error: Some(e),
            backtrace: backtrace,
        }
